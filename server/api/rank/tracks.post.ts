...
export default defineEventHandler(async (event) => {
  const body = await readBody<{ platform?: string; boardId?: string; page?: number; refresh?: boolean }>(event)
  if (!body?.boardId) {
  ...
  const result = await getRankTracks(platform, String(body.boardId), page, body?.refresh === true)
